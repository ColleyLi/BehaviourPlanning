    ...
    Frame* const frame, ReferenceLineInfo* const reference_line_info) 
{
  ...
  *(reference_line_info->mutable_path_data()) = valid_path_data.front();
  reference_line_info->SetBlockingObstacleId(valid_path_data.front().blocking_obstacle_id());
  ...
  if (!(reference_line_info->GetBlockingObstacleId()).empty()) 
  {
    if (PlanningContext::Instance()
            ->path_decider_info().front_static_obstacle_cycle_counter() < 0) 
    {
  ...
  if (PlanningContext::Instance()
      ...
      PlanningContext::Instance()
              ->path_decider_info().decided_side_pass_direction() == 0) 
  {
    if (reference_line_info->path_data().path_label().find("left") != std::string::npos) 
    {
      PlanningContext::Instance()
          ->mutable_path_decider_info()
          ->set_decided_side_pass_direction(1);
    } 
    else if (reference_line_info->path_data().path_label().find("right") != std::string::npos) 
    {
      PlanningContext::Instance()
          ->mutable_path_decider_info()
          ->set_decided_side_pass_direction(-1);
