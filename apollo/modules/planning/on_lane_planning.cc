                                 ...
                                 const VehicleState& vehicle_state) 
{
  ...
  {
    if (!ref_line.Shrink(Vec2d(vehicle_state.x(),
                               vehicle_state.y()),
                               FLAGS_look_backward_distance,
                               forward_limit)) 
    {
      std::string msg = "Failed to shrink reference line.";
      return Status(ErrorCode::PLANNING_ERROR, msg);
    }
  }
  ...
  for (auto& seg : segments) 
  {
    ...
    {
      std::string msg = "Failed to shrink routing segments.";
      return Status(ErrorCode::PLANNING_ERROR, msg);
    }
  }
                             ...
                             ADCTrajectory* const ptr_trajectory_pb) 
{
  ...
  {
    std::string msg("Update VehicleStateProvider failed or the vehicle state is outdated");
    AERROR << msg;
  ...
  if (FLAGS_estimate_current_vehicle_state &&
      start_timestamp - vehicle_state_timestamp < FLAGS_message_latency_threshold) 
  {
    vehicle_state = AlignTimeStamp(vehicle_state, start_timestamp);
  }

  // Update routing response if changed and save old routing
  ...
  // If somethig is wrong, fill ADCTrajectory with Estop trajectory and return
  if (!status.ok()) 
  {
  ...
  ADEBUG << "Total planning time: " << time_diff_ms << " ms";
    ...
    ADCTrajectory* const ptr_trajectory_pb) 
{
  ...
  // Selects Drivable reference line with min cost
...
bool OnLanePlanning::CheckPlanningConfig(const PlanningConfig& config)
{
  ...
  if (config.standard_planning_config().has_planner_scenarios())
  {
      if (config.standard_planning_config().planner_scenarios().scenario_type_size() == 0) 
      {
        return false;
      }
  }
