...
{
  if (PlanningContext::Instance()->path_decider_info().is_in_path_lane_borrow_scenario()) 
  {
    // If originally borrowing neighbor lane:
    if (PlanningContext::Instance()->path_decider_info().able_to_use_self_lane_counter() >= 6) 
    {
  ...
  else 
  {
    ...
    PlanningContext::Instance()->path_decider_info().front_static_obstacle_id();
        ...
        IsSidePassableObstacle(reference_line_info)) 
    {
      ...
      PlanningContext::Instance()->mutable_path_decider_info()->set_is_in_path_lane_borrow_scenario(true);
      AINFO << "Switch from SELF-LANE path to LANE-BORROW path.";
    }
  }

  return PlanningContext::Instance()->path_decider_info().is_in_path_lane_borrow_scenario();
}
