...
// #include "modules/planning_btree/common/planning_context.h"
#include "modules/planning_btree/common/btree_planning_gflags.h"
// #include "modules/routing/common/routing_gflags.h"

/**
 * @namespace apollo::planning_btree
 * @brief apollo::planning_btree
 */
namespace apollo {
...
ReferenceLineProvider::ReferenceLineProvider(
  ...
  ACHECK(cyber::common::GetProtoFromFile(FLAGS_btree_smoother_config_filename,
                                         &smoother_config_))
      << "Failed to load smoother config file "
      << FLAGS_btree_smoother_config_filename;
...
bool ReferenceLineProvider::Start() {
  ...
  if (FLAGS_btree_enable_reference_line_provider_thread) {
...
bool ReferenceLineProvider::CreateRouteSegments(
  ...
  if (FLAGS_btree_prioritize_change_lane) {
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  if (is_new_routing || !FLAGS_btree_enable_reference_line_stitching) {
...
bool ReferenceLineProvider::ExtendReferenceLine(const VehicleState &state,
  ...
  double future_start_s =
      std::max(sl_point.s(), prev_segment_length -
                                 FLAGS_btree_reference_line_stitch_overlap_distance);
  double future_end_s =
      prev_segment_length + FLAGS_btree_look_forward_extend_distance;
...
bool ReferenceLineProvider::IsReferenceLineSmoothValid(
  ...
  for (double s = 0.0; s < smoothed.Length();
    ...
    if (diff > FLAGS_btree_smoothed_reference_line_max_diff) {
...
bool ReferenceLineProvider::SmoothPrefixedReferenceLine(
    ...
    ReferenceLine *reference_line) {
  if (!FLAGS_btree_enable_smooth_reference_line) {
