...
#include "modules/planning_btree/reference_line/spiral_reference_line_smoother.h"
 ...
 public:
  ...
  static bool Smooth(std::vector<Eigen::Vector2d> raw_points,
    ...
    ACHECK(cyber::common::GetProtoFromFile(
        "modules/planning_btree/conf/spiral_smoother_config.pb.txt", &config));
...
int main(int argc, char* argv[]) {
  ...
  std::vector<Eigen::Vector2d> raw_points =
      apollo::planning_btree::SpiralSmootherUtil::ReadCoordinatesFromFile(
          FLAGS_input_file);

  std::vector<apollo::common::PathPoint> smooth_points;
  auto res =
      apollo::planning_btree::SpiralSmootherUtil::Smooth(raw_points, &smooth_points);
  if (!res) {
  ...
  apollo::planning_btree::SpiralSmootherUtil::Export(FLAGS_output_file,
                                               smooth_points);
  return 0;
}
