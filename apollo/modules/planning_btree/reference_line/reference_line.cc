...
std::string ReferenceLine::DebugString() const {
  const auto limit =
      std::min(reference_points_.size(),
               static_cast<size_t>(FLAGS_btree_trajectory_point_num_for_debug));
  return absl::StrCat(
...
double ReferenceLine::GetSpeedLimitFromS(const double s) const {
  ...
  double speed_limit = FLAGS_btree_planning_upper_speed_limit;
  ...
  if (!speed_limit_found) {
    ...
    speed_limit = FLAGS_btree_default_city_road_speed_limit;
    hdmap::Road::Type road_type = GetRoadType(s);
    if (road_type == hdmap::Road::HIGHWAY) {
      speed_limit = FLAGS_btree_default_highway_speed_limit;
    }
  }
