...
static constexpr bool kEnableMultiThreadInDpStGraph = true;
static constexpr bool  kUseStDrivableBoundary = true;
static constexpr double kPlanningUpperSpeedLimit = 20.0;
static constexpr double kUnitT = 1;
static constexpr double kDenseUnitS = 0.1;
static constexpr double kSparseUnitS = 1.0;
static constexpr double kDenseDimensionS = 101.0;
static constexpr double kMaxAcceleration = 4.5;
static constexpr double kMaxDeceleration = -4.5;
...
GriddedPathTimeGraph::GriddedPathTimeGraph(
    ...
    : st_graph_data_(st_graph_data),
      ...
      dp_st_cost_(st_graph_data_.total_time_by_conf(),
  ...
  for (size_t c = 0; c < cost_table_.size(); ++c) {
    ...
    if (count > 0) {
      ...
      for (size_t r = next_lowest_row; r <= next_highest_row; ++r) {
        ...
        if (kEnableMultiThreadInDpStGraph) {
          results.push_back(
...
void GriddedPathTimeGraph::CalculateCostAt(
  ...
  const double pre_lowest_s =
      cost_cr.point().s() -
      kPlanningUpperSpeedLimit * (1 + kSpeedRangeBuffer) * unit_t_;
