...
constexpr bool kEnableDpReferenceSpeed = true;
constexpr double kSpeedLonDecisionHorizon = 200.0;

constexpr double kSafeDistance = 20.0;
constexpr double kObstacleWeight = 1.0;
constexpr double kDefaultObstacleCost = 1e4;
constexpr double kSpatialPotentialPenalty = 1.0e2;
constexpr double kReferenceWeight = 0.0;
constexpr double kKeepClearLowSpeedPenalty = 10.0;
constexpr double kDefaultSpeedCost = 1.0e3;
constexpr double kLowSpeedPenalty = 10.0;
constexpr double kExceedSpeedPenalty = 10.0;
constexpr double kReferenceSpeedPenalty =  10.0;
constexpr double kMaxAcceleration = 4.5;
constexpr double kMaxDeceleration = -4.5;
constexpr double kAccelPenalty = 1.0;
constexpr double kDecelPenalty = 1.0;
constexpr double kPositiveJerkCoeff = 1.0;
constexpr double kNegativeJerkCoeff = 1.0; 
}

DpStCost::DpStCost(const double total_t,
...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  for (const auto* obstacle : obstacles_) {
    ...
    if (boundary.min_s() > kSpeedLonDecisionHorizon) {
      continue;
    }
    ...
    if (s < s_lower) {
      const double follow_distance_s = kSafeDistance;
      if (s + follow_distance_s < s_lower) {
        continue;
      } else {
        auto s_diff = follow_distance_s - s_lower + s;
        cost += kObstacleWeight * kDefaultObstacleCost *
                s_diff * s_diff;
      }
    } else if (s > s_upper) {
...
double DpStCost::GetSpatialPotentialCost(const StGraphPoint& point) {
  return (total_s_ - point.point().s()) * kSpatialPotentialPenalty;
}

double DpStCost::GetReferenceCost(const STPoint& point,
                                  const STPoint& reference_point) const {
  return kReferenceWeight * (point.s() - reference_point.s()) *
...
double DpStCost::GetSpeedCost(const STPoint& first, const STPoint& second,
  ...
  if (speed < max_adc_stop_speed && InKeepClearRange(second.s())) {
    // first.s in range
    cost += kKeepClearLowSpeedPenalty * unit_t_ *
            kDefaultSpeedCost;
  }

  double det_speed = (speed - speed_limit) / speed_limit;
  if (det_speed > 0) {
    cost += kExceedSpeedPenalty * kDefaultSpeedCost *
            (det_speed * det_speed) * unit_t_;
  } else if (det_speed < 0) {
    cost += kLowSpeedPenalty * kDefaultSpeedCost *
            -det_speed * unit_t_;
  }

  if (kEnableDpReferenceSpeed) {
    double diff_speed = speed - cruise_speed;
    cost += kReferenceSpeedPenalty * kDefaultSpeedCost *
...
double DpStCost::GetAccelCost(const double accel) {
  ...
  if (accel_cost_.at(accel_key) < 0.0) {
    ...
    double max_acc = kMaxAcceleration;
    double max_dec = kMaxDeceleration;
    double accel_penalty = kAccelPenalty;
    double decel_penalty = kDecelPenalty;
...
double DpStCost::JerkCost(const double jerk) {
  ...
  if (jerk_cost_.at(jerk_key) < 0.0) {
    ...
    if (jerk > 0) {
      cost = kPositiveJerkCoeff * jerk_sq * unit_t_;
    } else {
      cost = kNegativeJerkCoeff * jerk_sq * unit_t_;
    }
