...
void STObstaclesProcessor::Init(const double planning_distance,
  ...
  obstacle_decisions_ = obstacle_decisions;
...
bool STObstaclesProcessor::MapObstaclesToSTBoundaries(
    ObstacleDecisions* const obstacle_decisions) {
  // Sanity checks.
  if (obstacle_decisions == nullptr) {
    const std::string msg = "obstacle_decisions is nullptr";
    AERROR << msg;
    return false;
  }
  ...
  for (const auto* obs_item_ptr : obstacle_decisions->obstacles().Items()) {
    // Sanity checks.
    Obstacle* obs_ptr = obstacle_decisions->Find(obs_item_ptr->Id());
    if (obs_ptr == nullptr) {
...
bool STObstaclesProcessor::GetSBoundsFromDecisions(
  ...
  for (const auto& obs_id : obs_id_to_remove) {
    ...
    if (obs_id_to_alternative_st_boundary_.count(obs_id) > 0) {
      Obstacle* obs_ptr = obstacle_decisions_->Find(obs_id);
