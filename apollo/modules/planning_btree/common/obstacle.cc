...
const bool kUseNavigationMode = false;
const double kMinStopDistanceObstacle = 6.0;
const double kMaxStopDistanceObstacle = 10.0;
const double kVirtualStopWallHeight = 2.0;
const double kStMaxT = 8.0;
const double kNonstaticObstacleNudgeLBuffer = 0.4;
const double kStaticObstacleNudgeLBuffer = 0.3;
}  // namespace
...
std::unique_ptr<Obstacle> Obstacle::CreateStaticVirtualObstacles(
  ...
  perception_obstacle.set_height(kVirtualStopWallHeight);
...
double Obstacle::MinRadiusStopDistance(
  ...
  stop_distance = std::min(stop_distance, kMaxStopDistanceObstacle);
  stop_distance = std::max(stop_distance, kMinStopDistanceObstacle);
...
void Obstacle::BuildReferenceLineStBoundary(const ReferenceLine& reference_line,
  ...
  if (is_static_ || trajectory_.trajectory_point().empty()) {
    ...
    point_pairs.emplace_back(STPoint(start_s - adc_start_s, kStMaxT),
                             STPoint(end_s - adc_start_s, kStMaxT));
...
bool Obstacle::BuildTrajectoryStBoundary(const ReferenceLine& reference_line,
  ...
  for (int i = 1; i < trajectory_points.size(); ++i) {
    ...
    bool has_low = false;
    double high_s =
        std::min(object_boundary.end_s() + adc_half_length, kStMaxT);
    bool has_high = false;
      ...
      if (!has_low) {
        ...
        has_low = object_moving_box.HasOverlap(
            {low_ref, low_ref.heading(), adc_length,
             adc_width + kNonstaticObstacleNudgeLBuffer});
...
void Obstacle::CheckLaneBlocking(const ReferenceLine& reference_line) {
  ...
  if (reference_line.IsOnLane(sl_boundary_) &&
      driving_width <
          vehicle_param.width() + kStaticObstacleNudgeLBuffer) {
    is_lane_blocking_ = true;
    return;
  }
