...
double DestinationDistanceMetric(const TopoNode* cur_node, const TopoNode* dest_node)
{
  const auto& cur_point = cur_node->AnchorPoint();
  const auto& dest_point = dest_node->AnchorPoint();
  double distance = std::fabs(cur_point.x() - dest_point.x()) +
                    std::fabs(cur_point.y() - dest_point.y());
  return distance;
}

double GetCostToNeighbor(const TopoEdge* edge, const TopoNode* cur_node, const TopoNode* dest_node)
{
  double ratio = 1.0;
